...
checkDownload() {
  ...
  if [ "${nginx_option}" == '4' ]; then
	...
	mv caddy_${caddy_ver}_src.tar.gz caddy_${caddy_ver}.tar.gz
  fi
  ...
  case "${tomcat_option}" in
    1)
	  ...
	  src_url=https://archive.apache.org/dist/tomcat/tomcat-11/v${tomcat11_ver}/src/apache-tomcat-${tomcat11_ver}-src.tar.gz && Download_src
	  mv apache-tomcat-${tomcat11_ver}-src.tar.gz apache-tomcat-${tomcat11_ver}.tar.gz
    ...
    2)
	  ...
	  mv apache-tomcat-${tomcat10_ver}-src.tar.gz apache-tomcat-${tomcat10_ver}.tar.gz
      ;;
    3)
	  ...
	  mv apache-tomcat-${tomcat9_ver}-src.tar.gz apache-tomcat-${tomcat9_ver}.tar.gz
      ;;
    4)
	  ...
	  mv apache-tomcat-${tomcat8_ver}-src.tar.gz apache-tomcat-${tomcat8_ver}.tar.gz
      ;;
    5)
	  ...
	  mv apache-tomcat-${tomcat7_ver}-src.tar.gz apache-tomcat-${tomcat7_ver}.tar.gz
      ;;
  esac
  ...
  if [[ "${db_option}" =~ ^[1-9]$|^1[0-4]$ ]]; then
    if [[ "${db_option}" =~ ^[1,2,5,6,7,9]$|^10$ ]] && [ "${dbinstallmethod}" == "2" ]; then
      ...
      echo "Download boost..."
      # [ "${OUTIP_STATE}"x == "China"x ] && DOWN_ADDR_BOOST=${mirror_link}/oneinstack/src || 
  ...
  if [ "${php_option}" == '1' ] || [ "${mphp_ver}" == '53' ]; then
    ...
    src_url=https://github.com/P-H-C/phc-winner-argon2/archive/refs/tags/${argon2_ver}.tar.gz && Download_src
    mv ${argon2_ver}.tar.gz argon2-${argon2_ver}.tar.gz
    src_url=http://download.libsodium.org/libsodium/releases/libsodium-${libsodium_up_ver}.tar.gz && Download_src
    src_url=https://libzip.org/download/libzip-${libzip_ver}.tar.gz && Download_src
    ...
    # src_url=${mirror_link}/oneinstack/src/libsodium-${libsodium_up_ver}.tar.gz && Download_src
    # src_url=${mirror_link}/oneinstack/src/libzip-${libzip_ver}.tar.gz && Download_src
	src_url=https://www.php.net/distributions/php-${php84_ver}.tar.gz && Download_src
    src_url=https://github.com/P-H-C/phc-winner-argon2/archive/refs/tags/${argon2_ver}.tar.gz && Download_src
    mv ${argon2_ver}.tar.gz argon2-${argon2_ver}.tar.gz
    src_url=http://download.libsodium.org/libsodium/releases/libsodium-${libsodium_up_ver}.tar.gz && Download_src
    src_url=https://libzip.org/download/libzip-${libzip_ver}.tar.gz && Download_src
  ...
  if [ "${pecl_zendguardloader}" == '1' -a "${armplatform}" != 'y' ]; then
    case "${php_option}" in
      4)
		...
		src_url=http://downloads.zend.com/guard/7.0.0/zend-loader-php5.6-linux-x86_64.tar.gz && Download_src
        ;;
      3)
		...
		src_url=http://downloads.zend.com/guard/7.0.0/zend-loader-php5.5-linux-x86_64.tar.gz && Download_src
        ;;
      2)
  ...
  if [ "${memcached_flag}" == 'y' ]; then
    ...
    # [ "${OUTIP_STATE}"x == "China"x ] && DOWN_ADDR=${mirror_link}/oneinstack/src || 
